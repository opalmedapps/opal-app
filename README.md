...
for development and testing purposes. The app is built with [Cordova](https://cordova.apache.org/), which is a
...
In terms of the overall architecture, the app installed via this guide will communicate
with the development environment operated by O-HIG.
Instructions are also provided for connecting the app to a local backend installed on your personal development machine.
...
2. Change directory to the qplus folder and make sure the main branch is checked out.
...
5. Test the installation on your localhost.
   ...
   A few notes:
    - Passing `dev` to the `--env` command-line argument will make the app communicate with the development environment.
...
7. (Optional) If setting up a local development environment of Opal with your own backend system, you'll want
   to redirect Opal to connect to this backend instead of the development environment. In this case, follow the instructions at
   [env/README.md](./env/README.md) to set up your installation to use the `local` environment and start the app using `--env=local`.

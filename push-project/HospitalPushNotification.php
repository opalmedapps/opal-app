   ...
   class HospitalPushNotification{
      ...
      /**
       ...
       public function sendCallPatientNotification($patientId, $room, $appointmentAriaSer)
       {
           ...
           try{
                $sql = "UPDATE Appointment SET RoomLocation = '".$room."' WHERE Appointment.AppointmentAriaSer = ".$appointmentAriaSer;
           ...
           //Insert into notifications table, here it assumes the NotificationControlSerNum  = 10;
           try{
             $sql = 'INSERT INTO `Notification`(`NotificationSerNum`, `PatientSerNum`, `NotificationControlSerNum`, `RefTableRowSerNum`, `DateAdded`, `ReadStatus`, `LastUpdated`) VALUES (NULL,'.$result[0]["PatientSerNum"].',10,'.$result[0]["AppointmentSerNum"].', NOW(),0,NULL)';
           ...
           //Obtain NotificationSerNum for the last inserted Id.
            ...
            try{
                $sql = 'SELECT Name_'.$language.', Description_'.$language.' FROM NotificationControl WHERE NotificationControlSerNum = 10';
           ...
           //Obtain patient device identifiers
            ...
            {   
                ...
                self::pushNotificationDatabaseUpdate($device["PatientDeviceIdentifierSerNum"], $notificationSerNum, $response["success"], 1);
                //Build response
       ...
       **/
        ...
        *    and send status, $sendStatus, where send status is a 1 or 0 for whether is was successfully sent or not.
        *    Inserts a into the PushNotification table or updates SendLog flag.
        *    RegistrationId.
        *    Returns: Returns the send status   
        **/
       private function pushNotificationDatabaseUpdate($deviceSerNum, $notificationSerNum, $sendStatus)
       {
           global $pdo;
           $sql = " INSERT INTO `PushNotification`(`PushNotificationSerNum`,
                    `PatientDeviceIdentifierSerNum`, `NotificationSerNum`, `DateAdded`, `SendStatus`,
                    `SendLog`, `LastUpdated`) 
                    VALUES (NULL,".$deviceSerNum.", ".$notificationSerNum.",NOW(),".$sendStatus.",1, NULL)
                    ON DUPLICATE KEY UPDATE SendLog = SendLog + 1";
           $result = $pdo->query($sql);
           return $sendStatus;
       }

...
/**
 * Zhengbo Wang, combined with the work of Martina Marien
 * Date:
 */

// For now, this assumes that all graphs are 0-100 range.
// To show questionnaires with different max scores, it needs modification.

var app = angular.module('MUHCApp');
app.controller('globalScoreHistoryController', ['$scope', '$rootScope', 'Questionnaires', 'UserPreferences', '$location', 'NavigatorParameters', '$filter', '$timeout', function($scope, $rootScope, Questionnaires, UserPreferences, $location, NavigatorParameters, $filter, $timeout) {
    ...
    function activate() {
        ...
        vm.questionnaireScores = params.questionnaireScores;
        vm.questionnaireNickname = params.questionnaireNickname;
        // vm.algorithms = [default_alg].concat(params.algorithms);
        console.log(vm.algorithms);
        importantInfo();
        configureCharts();
        $scope.loading = false;
        //vm.algorithms = [defaultAlg].concat(params.algorithms);
        // Questionnaires.requestEntityScores(entity.ser_num, default_alg.ser_num)
        //     .then(function () {
    ...
    function drawLineChart() {
        var reformedData = [];
        for(var i = 0; i < vm.questionnaireScores.length; i++)
        {
            var dv = [];  //array to store pairs of [date, testResult]
            dv[0] = Date.parse(vm.questionnaireScores[i].last_updated);  //dateArray[0] = most recent date
            dv[1] = parseFloat(vm.questionnaireScores[i].score);
    ...
    function drawHistogram() {
        ...
        for(var i = 0; i < vm.questionnaireScores.length; i++)
        {
            var category = Math.floor(vm.questionnaireScores[i].score / 10);
            switch(category) {
                case 0:
                    reformedData[0]++;
                    break;
